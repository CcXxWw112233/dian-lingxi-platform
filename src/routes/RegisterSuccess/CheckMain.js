...
class CheckMain extends React.Component {
  ...
  render() {
    ...
    const { email = '', mobile = '', type = '', loadFlag = false, verifyResult } = datas
    ...
    return (
      <div>
        ...
        <div style={{ color: '#40A9FF', fontSize: 24, textAlign: 'center', marginTop: 48 }}>注册成功</div>
        <div style={{ color: 'rgba(0,0,0,.45)', fontSize: 16, textAlign: 'center', marginTop: 10 }}>{count_down}秒后自动跳转到登录页</div>
        <div className={indexStyles.buttonOuter}>
          <Button style={{ height: 40 }} type={'primary'} onClick={routingJump.bind(null, '/login')}>立即登录</Button>
        </div>
      </div>
    );
  }
