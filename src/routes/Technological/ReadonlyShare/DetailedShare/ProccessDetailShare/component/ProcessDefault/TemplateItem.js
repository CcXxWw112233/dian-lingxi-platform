...
export default class TemplateItem extends React.Component {
  ...
  render() {
    ...
    return (
      <div
        className={indexStyles.tem_item}
        style={{ paddingBottom: hasMore ? 0 : 12 }}
      >
        <div className={indexStyles.tem_item_top}>
          <div
            className={`${indexStyles.tem_item_l} ${globalStyles.authTheme}`}
          >
            &#xe605;
          </div>
          ...
          {checkIsHasPermissionInBoard(PROJECT_FLOWS_FLOW_TEMPLATE) && (
            <Tooltip title="删除模板">
              <div
                className={`${indexStyles.tem_item_r} ${globalStyles.authTheme} ${indexStyles.itemOperate}`}
                onClick={this.deleteTemplate.bind(this, { id })}
              >
                &#xe623;
              </div>
            </Tooltip>
          ...
          {checkIsHasPermissionInBoard(PROJECT_FLOWS_FLOW_CREATE) && (
            <Tooltip title="启动流程">
              <div
                className={`${indexStyles.tem_item_r} ${globalStyles.authTheme}  ${indexStyles.itemOperate}`}
                onClick={this.templateStartClick.bind(this, { id })}
              >
                &#xe61f;
              </div>
            </Tooltip>
        ...
        <div className={indexStyles.tem_item_bott} ref={'tempItemBott'}>
          <div
            className={`${indexStyles.tem_item_flow} ${
              globalStyles.authTheme
            } ${hasMore && !showBott ? indexStyles.tem_item_flow_hasMore : ''}`}
          >
            {flow_template_node_list.map((value, key) => {
              ...
              return (
                <div className={indexStyles.tem_item_flow_item} key={id}>
                  {name}{' '}
                  {key < flow_template_node_list.length - 1 && (
        ...
        {hasMore && (
          <div
            className={`${indexStyles.tem_item_bott_down} ${
              globalStyles.authTheme
            } ${
              showBott
                ? indexStyles.tem_item_bott_down_up
                : indexStyles.tem_item_bott_down_down
            }`}
            onClick={this.setShowBott.bind(this)}
          >
            &#xe7f0;
          </div>
        )}
