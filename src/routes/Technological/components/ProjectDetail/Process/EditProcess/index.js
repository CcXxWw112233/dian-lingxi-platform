...
//       "enable_opinion":"1",//是否填写意见  1=填写 0=不填写
//       "requires_data":{
//         "limit_file_num":"2",//限制文件上传数量 0=不限制
//         "limit_file_type":"1,2,3,4",//限制上传类型(文件格式)1=文档 2=图像 3=音频 4=视频
//         "limit_file_size":"30"//限制文件大小
//       }
//     }
//     break
//   case '3':
...
//       "approve_value":"50",//汇签值 当approve_type=3 时该字段有效
//       "deadline_type":"1",//完成期限类型 1=无期限 2=启动流程时指定 3=固定天数
//       "deadline_value":"3",//完成期限值
//       "assignee_type":"1",//审批人类型 1=任何人 2=启动流程时指定 3=固定人选
//       "assignees":"2131,421",//审批人(id) 多个逗号隔开
//       "transfer_mode":"1",//流转方式 1=自由选择 2= 下一步
//       "enable_revocation":"1",//是否可撤回 1=可撤回 0=不可撤回
//       "enable_opinion":"1"//是否填写意见  1=填写 0=不填写
//     }
...
export default class EditProcess extends React.Component {
  ...
  addNode(node_type) { //添加每一项默认里程碑开始，当前步数跳到最新
    const { datas: { processEditDatas = [], currentEditStep = 0  } } = this.props.model
    const nodeObj = {
      "name":"编辑节点名称",//节点名称
      "node_type":"1",//节点类型：1代表里程碑节点
      "description":"",
    ...
    this.props.updateDatas({
      processEditDatas,
      currentEditStep: processEditDatas.length - 1,
      node_type: '1'
    })
  }
  ...
  render() {
    const { datas: { node_type = '1', processEditDatas = [], currentEditStep = 0, } } = this.props.model
      ...
      <div className={indexStyles.editProcessOut}>
        <div className={indexStyles.editProcessLeft}>
          ...
          {processEditDatas.map((value, key) => {
            return (
              <div className={currentEditStep === key? indexStyles.itemSelect :indexStyles.item} onClick={this.currentEditStepClick.bind(this, {value, key})}>
