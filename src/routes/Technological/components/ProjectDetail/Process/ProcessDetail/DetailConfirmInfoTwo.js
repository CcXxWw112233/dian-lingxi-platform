            ...
            <div className={indexStyles.ConfirmInfoOut_1_bott_right} >
              ...
              <div className={indexStyles.fileList}>
                {['11.jpg','22.png'].map((value, key) => {
                  return(
                    <span style={{cursor: 'pointer', lineHeight: '33px', marginRight: 40}} key={key}><i className={globalStyles.authTheme} style={{fontStyle: 'normal',fontSize: 22, color: '#1890FF', marginRight: 8, cursor: 'pointer' }} dangerouslySetInnerHTML={{__html: this.judgeFileType(value)}}></i>{value}</span>
                  )
                })}
              </div>
