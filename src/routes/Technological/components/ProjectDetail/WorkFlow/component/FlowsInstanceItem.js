...
export default class FlowsInstanceItem extends Component {
  ...
  render() {
    ...
    const { id, name, total_node_num, total_node_name, completed_node_num, deadline_value, is_privilege, status } = itemValue
    return (
      <div onClick={() => { this.handleProcessInfo(id) }} className={indexStyles.flowsInstanceItemContent}>
        ...
        {
          <span><DifferenceDeadlineType type="flowInstanceItem" itemValue={itemValue} /></span>
        }
      </div>
    )
  }
