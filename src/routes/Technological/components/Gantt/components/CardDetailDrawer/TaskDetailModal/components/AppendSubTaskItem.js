...
import { Icon, Dropdown, Tooltip, Popconfirm, DatePicker, message, Menu, Breadcrumb, Modal } from 'antd'
...
export default class AppendSubTaskItem extends Component {
  ...
  render() {
    const { childTaskItemValue, childDataIndex, dispatch, projectDetailInfoData: { data = [] }, drawContent = {}, boardFolderTreeData, projectDetailInfoData } = this.props
                  ...
                  <div className={`${appendSubTaskStyles.file_item_wrapper}`} key={fileInfo.id}>
                    <div className={`${appendSubTaskStyles.file_item} ${appendSubTaskStyles.pub_hover}`} onClick={(e) => this.openFileDetailModal(e, fileInfo)} >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div className={appendSubTaskStyles.file_info}>{showMemberName(fileInfo.create_by)} 上传于 {fileInfo.create_time && timestampFormat(fileInfo.create_time, "MM-dd hh:mm")}</div>
