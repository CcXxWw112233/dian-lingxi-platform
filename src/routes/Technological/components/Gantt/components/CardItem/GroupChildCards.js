...
export default class GroupChildCards extends Component {
    ...
    hanldleCardsPostion = (cards) => {
        const { parent_value: { start_time: parent_start_time }, ceiHeight, ceilWidth } = this.props
        const list = [...cards].map((item, key) => {
            ...
            const start_time = transformTimestamp(item.start_time) || transformTimestamp(item.due_time)
            const end_time = transformTimestamp(item.due_time) || transformTimestamp(item.start_time)
            const due_time = end_time
            const top = ceiHeight * key
            const time_span = (!due_time || !start_time) ? 1 : (Math.floor((due_time - start_time) / (24 * 3600 * 1000))) + 1 //正常区间内
            const width = time_span * ceilWidth
            const height = task_item_height
            const left = Math.floor((start_time - transformTimestamp(parent_start_time)) / (24 * 60 * 60 * 1000)) * ceilWidth //相对父任务最左边的天数
            const new_item = {
                ...item,
                top, due_time,
                start_time, end_time,
                is_has_end_time,
                is_has_start_time, time_span,
                width, height,
                left
            }
            return new_item
    ...
    renderEmpty = () => {
        return (
            <div className={styles.empty_out}>
                <div className={`${styles.empty} ${globalStyles.authTheme}`}>&#xe703; </div>
                <div>暂无数据</div>
            </div>
