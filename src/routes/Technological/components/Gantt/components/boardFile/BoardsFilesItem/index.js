...
export default class Index extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const that = this
    const { current_folder_id } = this.state
    const im_all_latest_unread_messages_new =
      nextProps.im_all_latest_unread_messages
    ...
    if (
      currentFolderJudegeFileUpload({
        folder_id: current_folder_id,
        im_all_latest_unread_messages: im_all_latest_unread_messages_new
      }) &&
      length_1 != length_2
    ) {
      setTimeout(() => {
    ...
    return (
      ...
      >
        ...
        {this.props.isInOpenFile &&
          board_id &&
          this.props.previewFileModalVisibile && (
            <FileDetailModal
              // {...this.props}
