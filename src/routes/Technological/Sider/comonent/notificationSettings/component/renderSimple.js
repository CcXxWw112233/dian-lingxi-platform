...
export default class renderSimple extends Component {
    ...
    render() {
        const { new_notice_list, new_simple_default_options } = this.props
        // console.log(new_simple_default_options, 'sssss_simple')
                    ...
                    new_notice_list && new_notice_list.map((item, index) => {
                        return (
                            <CommonOptions key={item.id} index={index} itemVal={item} 
                                default_options={new_simple_default_options} 
