...
import {
  ...
  generateBoardCode,
  inviteMemberJoinOrg,
  inviteMemberJoinBoard
} from '@/services/technological/noviceGuide'
import {
  scanQrCodeJoin,
...
export default class Boundary extends Component {
  state = {
    ...
    initInputList: [{ value: '' }, { value: '' }, { value: '' }], // 初始化的input框
    inputList: [{ value: '' }], // input框的
  ...
  handleNext = async () => {
    ...
    if (isApiResponseOk(res)) {
      this.setState(
        ...
        () => {
          this.getRoutineCode({
            boardId: res.data.data.board_id,
    ...
    return (
      <div className={styles.introduce}>
        ...
        <div className={styles.form}>
          ...
          <span onClick={this.handleAddMore} className={styles.add_more}>
            + 添加更多...
          </span>
          <div className={styles.code_wechat}>
            <span>{img_src && <img src={img_src} />}</span>
            <p>
              <b className={styles.line}></b>
              <i className={`${glabalStyles.authTheme} ${styles.wechat}`}>
                &#xe634;
              </i>{' '}
              微信扫一扫直接邀请参与人
              <b className={styles.line}></b>
