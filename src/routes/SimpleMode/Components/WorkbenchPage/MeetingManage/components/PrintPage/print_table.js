...
export default class PrintTable extends React.PureComponent {
  state = {}
  // 渲染head
  renderThead = () => {
  ...
  render() {
    const { printData = {}, totalNumber, number } = this.props
    const { lessee_org = {} } = printData
    return (
      <div className={`${styles.prnt_body} prnt_body`}>
        <div className={`${styles.print_title} print_title`}>
          <div>{lessee_org.name}</div>
          <div>{(printData.title || '').replace(lessee_org.name, '')}</div>
        </div>
        <div className={`${styles.order_propertys} order_propertys`}>
          <Row type="flex">
              ...
              <span
                className={
                  printData.bill_status === '1'
                    ? `${styles.notsettlement} notsettlement`
                    : `${styles.settlement} settlement`
                }
              >
                {printData.bill_status === '1'
                  ? '未结算'
        ...
        <div className={`${styles.order_table} order_table`}>
          <table>
            <thead>
        ...
        <div className={`${styles.footer_page} footer_page`}>
          第 {number} 页/ 共 {totalNumber} 页
        </div>
      </div>
    )
  }
