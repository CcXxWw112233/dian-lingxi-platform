...
import {
  getTemporaryFileData,
  removeTemporaryFileData
} from '@/services/technological/temporaryFile'
...
class UploadTemporaryFile extends Component {
  ...
  render() {
    const { isVisibleFileList, dragEnterCaptureFlag } = this.props

    const { temporaryData, currentCheckedList } = this.state

    return (
      <div
        className={`${indexStyles.temporaryFile} ${
          isVisibleFileList ? indexStyles.changeContentWidth : null
        }`}
      >
        {/* 上传本地文件 */}
        ...
        {!this.state.previewFileModalVisibile && (
          <div
            className={`${indexStyles.draggerContainerStyle} ${
              isVisibleFileList ? indexStyles.changeDraggerWidth : null
            }`}
          >
            ...
            >
              <div
                className={`${indexStyles.indexCoverWapper} ${
                  dragEnterCaptureFlag ? indexStyles.draging : ''
                }`}
              >
                {dragEnterCaptureFlag ? (
                  <div className={indexStyles.iconDescription}>
                    ...
                    />
                    <span className={indexStyles.iconDescription}>
                      松开鼠标左键即可上传文件
                    </span>
                  </div>
                ) : (
