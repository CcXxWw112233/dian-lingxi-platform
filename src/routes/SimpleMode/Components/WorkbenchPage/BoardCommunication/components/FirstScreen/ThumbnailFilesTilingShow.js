    ...
    render(){
        ...
        return(
            <div className={styles.ThumbnailFilesTilingShow}>
                {
                    thumbnailFilesList && thumbnailFilesList.length !==0 && thumbnailFilesList.map(item=>{
                        return (
                            <div className={styles.itemBox} key={item.id} title={item.file_name} onClick={()=>this.props.previewFile(item)}>
                                {
                                    item.thumbnail_url ? (
                                        ...
                                        <div
                                            ...
                                            dangerouslySetInnerHTML={{ __html: filterFileFormatType(item.file_name) }}
                                        >
                                        </div>
                                    )
                                }
