...
export default class BoardItem extends Component {
    ...
    visitControlUpdateInGanttData = (obj = {}) => {
        ...
        const { dispatch, itemValue: { board_id }, projectList = [] } = this.props
        ...
        if (type == 'privilege') {
            ...
            projectList_new[index].privileges = [].concat(projectList_new[index].privileges, privileges[0])
        } else if (type == 'remove') {
            projectList_new[index].privileges = projectList_new[index].privileges.filter((item) => item.id != removeId)
        } else {
    ...
    renderMenuOperateListName = ({ board_id, is_star }) => {
        ...
        return (
            <Menu onClick={this.handleMenuSelect} >
                ...
                {/* 渲染分组|项目对应的访问控制 */}
                {
                    checkIsHasPermissionInBoard(PROJECT_TEAM_BOARD_CONTENT_PRIVILEGE, board_id) && renderVistorContorlVisible && (
                        <Menu.Item key={'visitorControl'}>
                            <div
                                // style={{ height: 60, width: 100, backgroundColor: 'red' }}
                                onClick={this.handleVisitorWrapper}
                                onMouseDown={() => { this.handleVisitorControlMouseDown(board_id) }}>
    ...
    render() {
        const { itemValue: { board_id, board_name, org_id, is_star }, simplemodeCurrentProject, currentUserOrganizes = [], currentSelectOrganize = {} } = this.props
