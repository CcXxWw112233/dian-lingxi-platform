...
export default class BoardFeaturesProcessItem extends Component {
    ...
    renderTime = () => {
        ...
        let time = ''
        let dec = ''
        if (deadline_type == '1') { // 表示未限制时间
            return {
                time: '',
                dec: '未限制时间'
            }
        } else if (deadline_type == '2') { // 表示限制时间
            return {
                time: is_today(last_complete_time) ? `今天 ${timestampToHM(last_complete_time)}` : timestampToTimeNormal(last_complete_time, '/', true),
                dec: '截止'
            }
        }
    }
            ...
            <div className={`${!!belong_name ? styles.feature_item2 : styles.feature_item}`} onClick={this.itemClick}>
                ...
                <div className={`${styles.feature_item_middle}  ${globalStyles.global_ellipsis}`}>
                    <div className={`${styles.feature_item_middle_name}  ${globalStyles.global_ellipsis}`}>
                        <span className={`${globalStyles.authTheme}`}>&#xe68b;</span>
                        <span>
                            {name} <span className={globalStyles.authTheme}>&#xe7f0;</span> {total_node_name} {`( ${completed_node_num} / ${total_node_num} )`}
                        </span>
                    </div>
                    {!!belong_name && (
