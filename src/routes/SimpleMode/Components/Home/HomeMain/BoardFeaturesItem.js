...
import { timestampToTimeNormal, timeColor, isSamDay, transformTimestamp } from '../../../../../utils/util'
...
export default class BoardFeaturesItem extends Component {
    ...
    renderTime = () => {
        ...
        if (rela_type == '2') {
            return {
                time: is_today(due_time) ? '今天' : timestampToTimeNormal(start_time, '/', true),
                dec: '开始'
            }

        } else {
            return {
                time: is_today(due_time) ? '今天' : timestampToTimeNormal(due_time, '/', true),
                dec: '截止'
            }
        }
    }
