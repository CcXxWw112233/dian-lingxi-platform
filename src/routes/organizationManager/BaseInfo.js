...
import { Input, Radio, Button, message, Upload, Icon, Row, Col, Table, Tooltip } from 'antd'
import {
...
export default class BaseInfo extends React.Component {
  ...
  orderListColumns = [
    ...
    },
    {
      ...
      render: (record) => {
        let statuName = '';
        if (record.status == 0) {
          statuName = <a href='/pay/' target='_blank'>待支付</a>;
  ...
  render() {
    ...
    const { datas: { currentOrganizationInfo = {}, paymentInfo = {}, payOrderList = [] } } = this.props.model
            ...
            paymentInfo.is_free_trial && paymentInfo.is_free_trial === '0' && (
              <div className={indexStyles.paymemtInfo}>
                <div className={indexStyles.info}>
                  ...
                  <Row className={indexStyles.item}>
                    <Col span={4} className={indexStyles.itemLabel}>项目数量</Col>
                    <Col span={20} className={indexStyles.itemInfo}>{paymentInfo.board_number}</Col>
                  </Row>
                  <Row className={indexStyles.item}>
                    <Col span={4} className={indexStyles.itemLabel}>成员数量</Col>
                    <Col span={20} className={indexStyles.itemInfo}>
                      {parseInt(paymentInfo.member_number) >= parseInt(paymentInfo.member_number_limit) ?
                  ...
                  <Row className={indexStyles.item}>
                    <Col span={4} className={indexStyles.itemLabel}>到期时间</Col>
                    <Col span={20} className={indexStyles.itemInfo}>
                      {isOverdueTime(paymentInfo.expire_date) ?
                        <span style={{ color: '#F5222D' }}>{timestampToTime(paymentInfo.expire_date, false)}</span>
                        :
                        <span>{timestampToTime(paymentInfo.expire_date, false)}</span>
                      }
                  ...
                  <Row style={{ marginTop: '10px' }}>
                    <Tooltip title="续费功能即将开通">
                      <Button type={'primary'} disabled={true}>续费</Button>
                    </Tooltip>

                    <Button className={indexStyles.orderBtn} onClick={() => { this.loadOrderList() }}>查看订单</Button>
                  </Row>
