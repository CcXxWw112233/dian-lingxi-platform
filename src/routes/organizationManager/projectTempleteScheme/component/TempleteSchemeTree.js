...
export default class TempleteSchemeTree extends Component {
  ...
  recursion = (data, currentId) => {
    ...
    // this.getCurrentElementParentKey(currentId, this.props.currentTempleteListContainer)
    // console.log(result, 'ssssssssss_result')
    return result
  }
  ...
  handleCreateTempContainer = (e) => {
    e && e.stopPropagation()
    const { currentSelectedItemInfo: { template_data_type, template_id, parent_id, id } } = this.props
  ...
  renderPlanTreeNode = (data) => {
    return data && data.map(item => {
      let { template_data_type, name, id } = item
      if (item.child_content && item.child_content.length > 0) {
        return (
          <TreeNode title={this.renderPlanTreeTitle({ type: template_data_type, name, id })} key={item.id} dataRef={item} >
            {this.renderPlanTreeNode(item.child_content)}
          </TreeNode>
        );
      } else {
        return <TreeNode title={this.renderPlanTreeTitle({ type: template_data_type, name })} key={item.id} dataRef={item} />;
      }
      ...
      <div className={indexStyles.planningSchemeItemWrapper}>
        ...
        <div className={indexStyles.planningSchemeItem_top}>
          <div>
            ...
            <span style={{ marginLeft: '18px' }} className={`${globalStyles.authTheme} ${flag && indexStyles.pub_hover}`}>&#xe6f2; 添加子级</span>
          </div>
          <div>
            <span style={{ marginRight: '18px' }} className={`${globalStyles.authTheme} ${flag && indexStyles.pub_hover}`}>&#xe602; 重命名</span>
