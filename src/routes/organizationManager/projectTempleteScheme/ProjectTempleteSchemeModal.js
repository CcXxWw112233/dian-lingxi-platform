...
export default class ProjectTempleteSchemeModal extends Component {
  ...
  renderTempSettings = (item) => {
    return (
      <div>
        <Menu onClick={(e) => {this.handleSelectSettings(e,item)}} getPopupContainer={triggerNode => triggerNode.parentNode}>
      ...
      <div className={`${indexStyles.plan_list_wrapper} ${globalStyles.global_vertical_scrollbar}`}>
        ...
        </div> */}
        {
          !(projectTemplateList && projectTemplateList.length) ? (
            <div style={{width: '100%',textAlign: 'center', color: 'rgba(0,0,0,0.45)'}}>暂无数据</div>
            ...
            return (
              template_type == '1' ? (
                <div key={item.id} style={{ position: 'relative', marginRight: '16px' }} className={indexStyles.margin_right} onClick={() => { this.handleOperatorSchemeList({ id, name }) }}>
                  ...
                  <span title={item.name} className={indexStyles.plan_default_name}>{item.name}</span>
                </div>
              ) : (
                  <div key={item.id} style={{ position: 'relative' }} className={`${indexStyles.margin_right} ${indexStyles.others_list}`} onClick={() => { this.handleOperatorSchemeList({ id, name }) }}>
                    {
                      is_rename ? (
                        <Input maxLength={50} autoFocus={true} value={inputValue} onClick={e=>e.stopPropagation()} onChange={this.handleChangeValue} onPressEnter={(e) => { this.handleChangeValueBlur(e,item.id,name) }} onBlur={(e) => { this.handleChangeValueBlur(e,item.id,name) }} style={{width: '124px', height: '32px', padding: '5px 4px', position: 'absolute', top: '50%', right: 0, left: 0, bottom: 0, transform: 'translateY(-25%)', margin: '0 auto'}} />
                      ) : (
    ...
    return (
      <div>
        ...
        <div className={indexStyles.title}>
          ...
          <div style={{ fontSize: '20px', fontWeight: 900, color: 'rgba(0,0,0,0.65)', letterSpacing: '4px' }}>
