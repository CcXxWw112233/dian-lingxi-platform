...
export default class OrgnizationRole extends React.Component {
  ...
  onCheck = (parentKey, e) => {
    const { datas: { orgnization_role_data }} = this.props.model
    orgnization_role_data[parentKey]['already_has_content_permission_trans'] = e
    this.props.updateDatas({
      orgnization_role_data
    })
  }
    ...
    const operateMenu = ({parentKey, value}) => {
      ...
      return (
        <Menu onClick={this.handleMenuClick.bind(this, {parentKey, value})}>
          {is_default !== '1' && is_visitor != '1' && checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_EDIT)?(
          ...
          {(is_visitor != '1' && checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_CREATE)) && (
          ...
          {is_default !== '1' && is_visitor != '1' && checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_DELETE) ?(
      ...
      <div className={indexStyles.TabPaneContent}>
        <Collapse accordion onChange={this.collapseOnchange.bind(this)}>
          {orgnization_role_data.map((value, parentKey) => {
            ...
            const checkDisabled = !checkIsHasPermission(ORG_UPMS_ORGANIZATION_ROLE_EDIT) || system_role ==='1'|| is_visitor == '1'
            return (
              <Panel header={
