...
export default class RenderContent extends Component {
    ...
    render() {
      const { historyList = [], rela_type, rela_id, is_add_remind, user_remind_info, commonExecutors = [], processEditDatas = [], milestonePrincipals = [] } = this.props;
    
      {
        return (
          <div className={infoRemindStyle.content}>
              ...
              {
                is_add_remind && <RenderAdd rela_type={rela_type} commonExecutors={commonExecutors} processEditDatas={processEditDatas} milestonePrincipals={milestonePrincipals} rela_id={rela_id} user_remind_info={user_remind_info} />
              }
          </div>
        ) 
      }
