...
export default class RenderAdd extends Component {
  ...
  componentDidMount() {
    ...
    if (rela_type == '1' || rela_type == '2') {
      // 表示在工作台或者项目详情中的任务或者日程弹窗中--才会进来
      if (commonExecutors && commonExecutors.length) {
        // 存在执行人
        let new_userRemindInfo = [...user_remind_info]
        new_userRemindInfo = new_userRemindInfo.filter(item => {
          let new_item = item
          if (new_item['user_id'] == '1') {
            // 表示找到了执行人
            return new_item
          }
        })
      ...
      if (new_assignees && new_assignees.length) {
        // 存在推进人
        let new_userRemindInfo = [...user_remind_info]
        new_userRemindInfo = new_userRemindInfo.filter(item => {
          let new_item = item
          if (new_item['user_id'] == '2') {
            // 表示找到了推进人
            return new_item
          }
        })
  ...
  handleSetInfoRemind() {
    ...
    new_info_list = new_info_list.map(item => {
      ...
      new_item = {
        ...new_item,
        is_edit_status: true,
        message_consumers: message_consumers
      }
      return new_item
    })
    let params = {}
        ...
        <div className={`${infoRemindStyle.select}`}>
          ...
          {remind_edit_type == 3 && (
            <DatePicker
              ...
              onOk={value => {
                this.handleDatePickerOk(value, remind_time_value)
              }}
            ...
            >
              {diff_remind_time.map(childItem => {
                return (
                  <Option
                    onClick={() => {
                      this.onDiffRemindTime(childItem.remind_time_value)
                    }}
            ...
            >
              {diff_text_term.map(childItem => {
                return (
                  <Option
                    onClick={() => {
                      this.onDiffTextTerm(childItem.remind_time_type)
                    }}
              ...
              overlay={
                <AddMembersExecutor
                  ...
                  multipleSelectUserChange={e =>
                    this.multipleUserSelectUserChange(e, message_consumers)
                  } //选择了某一项
                />
              }
            >
