...
export default class DragValidation extends React.Component{
  ...
  handleNeedX = (event ,type) => {
    ...
    }else if(type === 'up') { //鼠标放开, 拉到固定的位置表示验证成功了
      needX = this.state.needX === this.state.blockMaxLeft ? this.state.blockMaxLeft : 0
      this.setState({
  ...
  render(){
    ...
    return (
      <div className={indexStyles.dragOuter} ref="dragOuter">
        <div className={indexStyles.dragBack}>
          请按住滑块，拖动到最右边
        </div>
        ...
        <div className={indexStyles.dragSlideStrip} style={{ top: 0,
