...
export default class BeginningStepTwo extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      is_show_spread_arrow: props.itemValue.status == '1' || (props.itemKey == curr_position - 1) || (props.itemValue.runtime_type == '1') || (props.itemKey == (overrule_position != '' ? Number(overrule_position) + 1 : '')) || (props.itemKey == rating_position - 1) ? true : false,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (!isObjectValueEqual(this.props, nextProps)) {
      ...
      this.setState({
        is_show_spread_arrow: nextProps.itemValue.status == '1' || (nextProps.itemKey == curr_position - 1) || (nextProps.itemValue.runtime_type == '1') || (nextProps.itemKey == (overrule_position != '' ? Number(overrule_position) + 1 : '')) || (nextProps.itemKey == rating_position - 1) ? true : false,
                    ...
                    <div className={indexStyles.appListWrapper}>
                      <div className={indexStyles.app_left}>
                        ...
                        <div>
                          ...
                          <div style={{ color: 'rgba(0,0,0,0.25)' }}>{'(暂未审批)'}</div>
                        </div>
                      </div>
                      <div className={indexStyles.app_right}>{timestampToTimeNormal(time, '/', true) || ''}</div>
                    </div>
          ...
          <div style={{ marginTop: '12px' }}>
            ...
            }
            {
              (historyCommentsList && historyCommentsList.length != 0) && (
                <div>
                  <span><span style={{ fontSize: '16px' }} className={globalStyles.authTheme}>&#xe90e;</span> 历史审批:</span>
                  {
            ...
            {
              this.state.currentSelectArrow == itemKey && (historyCommentsList && historyCommentsList.length != 0) && historyCommentsList.map(item => {
