    ...
    switch (status) {
      case '1':
        if (deadline_type == '1') {
          ...
          return container = <span>当前步骤完成期限: <span style={{color: overdue_color ? '#F5222D': 'rgba(24,144,255,1)',marginLeft: '5px'}}><span>{renderRestrictionsTime(itemValue)}</span></span></span>
        }
      case '2':
    ...
    switch (status) {
      case '1':
        if (deadline_type == '1') {
          ...
          return container = <span style={{color: 'rgba(0,0,0,0.45)', display: 'flex'}}><span style={{display: 'flex', flexShrink: 0, lineHeight: '16px', marginRight: '5px'}} className={globalStyles.authTheme}><span style={{fontSize: '16px', fontWeight: 900, marginRight: '5px'}}>&#xe686;</span> 完成期限: </span><span style={{flexShrink: 0, lineHeight: '16px', color: overdue_color ? '#F5222D' : 'rgba(0,0,0,0.45)'}}>{renderRestrictionsTime(itemValue)}</span></span>
        }
        break
      case '2': // 表示已完成
