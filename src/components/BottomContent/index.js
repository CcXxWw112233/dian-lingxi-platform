  ...
  return (
    <div className={indexStyles.bottomWhole}>
      ...
      <div className={indexStyles.bottomWholeRight}>
        {type === 'login'? ( <p><span onClick={routingJump.bind(null, '/retrievePassword')}>忘记密码？</span><span>|</span><span onClick={routingJump.bind(null, '/register')}>注册账户</span></p>) : (<span onClick={routingJump.bind(null, '/login')}>已有账户，直接登陆</span>)}
      </div>
    </div>
  );
};
