    ...
    return (
      <div key={id} className={commonStyles.custom_field_item_wrapper}>
        <div className={commonStyles.custom_field_item}>
          ...
          <div id={`custom_field_value_${id}`} className={`${commonStyles.field_value} ${commonStyles.pub_hover}`}>
            <div className={commonStyles.common_select}>
              <span style={{ position: 'relative', zIndex: 0, minWidth: '80px', display: 'inline-block', color: value ? 'rgba(0,0,0,0.65)' : 'rgba(0,0,0,0.45)' }}>
                {value || '设置日期'}
                <DatePicker
                  // disabledDate={this.disabledStartTime.bind(this)}
                  // onOk={this.startDatePickerChange.bind(this)}
                  value={value ? moment(new Date(Number(field_value))) : undefined}
                  onChange={this.startDatePickerChange.bind(this)}
                  getCalendarContainer={() => document.getElementById(`custom_field_value_${id}`)}
                  // placeholder={start_time ? timestampToTimeNormal(start_time, '/', true) : '开始时间'}
                  format={this.getFormatter(date_field_code).formatterDatePicker}
