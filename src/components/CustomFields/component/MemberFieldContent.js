...
export default class MemberFieldContent extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      selected_memebers_value: props.itemValue && props.itemValue.field_value ? props.itemValue && props.itemValue.field_value : []
    }
  }
  ...
  handleRemoveExecutors = (e, user_id) => {
    e && e.stopPropagation()
    const { selected_memebers_value = [], itemValue: { id } } = this.state
    let users = selected_memebers_value.filter(item => item.user_id != user_id).map(item => {
      return item.user_id
    })
    this.props.dispatch({
  ...
  handleSingleMembersData = (data) => {
    ...
    if (type == 'add') { // 表示添加的操作
      ...
      const { selected_memebers_value = [], itemValue: { field_value = [] } } = this.state
      // 多个任务执行人
      let gold_user = field_value.filter(item => item.user_id != key)
      let user_id = !!(gold_user && gold_user.length) ? gold_user[0].user_id : ''
  ...
  chirldrenTaskChargeChange = (data) => {
    const { itemValue: { id, field_content: { field_set: { member_selected_type } } } } = this.state
              ...
              !(selected_memebers_value && selected_memebers_value.length) ? (
                <div className={`${commonStyles.field_value} ${commonStyles.pub_hover}`}>
                  ...
                  <div style={{ flex: '1', position: 'relative' }}>
                    ...
                    >
                      <div style={{ display: 'flex', flexWrap: 'wrap', minHeight: '38px' }} className={`${commonStyles.field_right} ${commonStyles.pub_hover}`}>
                        {selected_memebers_value.map((value) => {
                          const { avatar, name, user_name, user_id } = value
                          return (
                            <div key={user_id} className={`${commonStyles.first_pric}`} style={{ display: 'flex', flexWrap: 'wrap', marginLeft: '-12px' }} key={user_id}>
                              <div className={`${commonStyles.user_item}`} style={{ display: 'flex', alignItems: 'center', position: 'relative', margin: '2px 10px', textAlign: 'center', height: '38px' }} key={user_id}>
                                {avatar ? (
                                ...
                                <div style={{ marginRight: 8, fontSize: '14px' }} className={commonStyles.value_text}>{name || user_name || '佚名'}</div>
                                <span onClick={(e) => { this.handleRemoveExecutors(e, user_id) }} className={`${commonStyles.userItemDeleBtn}`}></span>
                              </div>
