...
import { Select, Dropdown, Menu, Icon, DatePicker, Input, InputNumber, Upload, Progress, message } from 'antd'
...
import { setUploadHeaderBaseInfo, getSubfixName } from '../../../utils/businessFunction'
...
export default class FileFieldContent extends Component {
  ...
  handleChange = ({ file, fileList, event }) => {
    const { itemValue: { id, field_value = [] } } = this.state
  ...
  getUploadProps = () => {
    ...
    const { projectDetailInfoData: { org_id, board_id } } = this.props;
    return {
      name: 'file',
  ...
  onFilePreview = (e, item) => {
    e && e.stopPropagation()
    const file_id = item.id || ((item.response && item.response.data) && item.response.data.id) || ''
  ...
  renderFileList = (item) => {
    const { fileList = [] } = this.state
    let gold_item_id = (item.status && item.status == 'done' && item.response && item.response.code == '0') && item.response.data.id
    ...
    let gold_normal_message = ((item.status && item.status == 'done' && item.response && item.response.data.code == '0') && item.response.data.file_name) || item.file_name
    const alrm_obj = {}
    if (status == 'error') {
      if (errorMsg) {
      ...
      <>
        <div key={item.uid || item.id} className={commonStyles.file_item} onClick={(e) => { this.onFilePreview(e, item) }}>
          ...
          <div style={{ flexShrink: 0 }}>
            <span
              onClick={(e) => { this.handleDeleteCustomFile(e, item.id || gold_item_id, item.uid) }} 
              className={commonStyles.del_name}>删除</span>
          </div>
        </div>
  ...
  render() {
    const { itemValue, itemKey, inputValue, setPreviewFileModalVisibile } = this.state
            ...
            <div className={`${commonStyles.field_value} ${commonStyles.pub_hover}`}>
              <Upload {...this.getUploadProps()}>
                <div onClick={this.onClick} className={commonStyles.common_select}>
                  <span style={{color: 'rgba(0,0,0,0.45)'}}>{'上传文件'}</span>
                </div>
              </Upload>
            </div>
            {
