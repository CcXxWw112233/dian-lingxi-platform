...
export default class UploadNormal extends Component {
  ...
  setUploadingFileList = (list = []) => {
    ...
    gold_arr = gold_arr.map(item => {
      //目标覆盖
      let new_item = { ...item }
      new_item = old_list.find(val => val.uid == new_item.uid) || new_item
      const { size, status, name, percent, uid, response } = new_item
      return {
        size,
        status,
        name,
  ...
  normalUploadProps = () => {
    ...
    const propsObj = {
      ...
      onChange: ({ file, fileList }) => {
        ...
        const is_has_uploading =
          fileList_will.length &&
          fileList_will.findIndex(item => item.status == 'uploading') != -1
        if (!is_has_uploading) {
          //没有上传状态了
          setTimeout(function() {
        ...
        if (file.status === 'done') {
          if (file.response && file.response.code == '0') {
            ...
            message.error(
              (file.response && file.response.message) || '上传出现了点问题'
            )
          }
        }
      },
  ...
  customRequest = async e => {
    ...
    if (file.size >= 100 * 1024 * 1024) {
      ...
      const {
        data: { is_live, access = {} }
      } = await this.checkFileMD5({ md5_str, file_name: file.name }) //检查后台是否存在相同md5的文件
      if (is_live) {
        //如果后端已经存在了该文件，只需调用接口将文件关联
        ...
        this.UploadToOss(params, { progressCalback, successCalback })
          .then(res => {
            that
              .checkRelationFile({
                md5_str,
  ...
  checkFileMD5 = ({ md5_str, file_name }) => {
    ...
    const {
      data: { board_id, folder_id, upload_type, file_version_id }
    } = uploadProps
    const params = {
      board_id,
      // folder_id,
  ...
  UploadToOss = (
    { access, file, hash, file_name },
    { progressCalback, successCalback }
  ) => {
    const url = this.uploadPath({ hash, file_name })
    return new Promise((resolve, reject) => {
      this.ossClient(access)
  ...
  render() {
    ...
    return (
      <>
        {swich_render_upload ? (
          <Upload {...this.normalUploadProps()}>{children}</Upload>
        ) : (
          children
        )}
        {/* {
...
UploadNormal.defaultProps = {
  ...
  setUploadingFileList: function() {
    //设置上传文件的列表(is_need_parent_notification == true时传入)
  },
  setUploadNotiVisible: function() {
    //关闭弹窗（is_need_parent_notification == true时传入)
  },
  uploadCompleteCalback: function() {
    //上传完成的回调，（比如查询列表）
  }
}
