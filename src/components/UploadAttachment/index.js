...
export default class UploadAttachment extends Component {
  ...
  renderFolderTreeNodes = data => {
    ...
    render() {
        ...
        const { uploadFileVisible, uploadFileList = [], toNoticeList=[]} = this.state;
        console.log("toNoticeList",toNoticeList);
        const { data:projectMemberData} = projectDetailInfoData;
        return (

            <div key={Math.random()}> 
                <Upload {...this.getUploadProps()} className={styles.uploadBtn}>
                    {children} 
                </Upload>
                ...
                >
                    ...
                    <TreeSelect
                            defaultValue={this.state.fileSavePath}
                            value={this.state.fileSavePath}
                            showSearch={false}
                            style={{ width: 508 }}
