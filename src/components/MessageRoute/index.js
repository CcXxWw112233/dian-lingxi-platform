...
export default class MessageRoute extends React.Component{
  ...
  render() {
    ...
    return(
      <span style={{color: '#000'}}>{discriptionText}{this.state.initTimeText}。<span style={{cursor: 'pointer'}} className={globalStyles.link_mouse} onClick={this.redirect}>{jumpText}</span></span>
    )
  }
};
