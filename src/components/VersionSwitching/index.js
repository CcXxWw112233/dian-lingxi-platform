...
export default class VersionSwitchingComponent extends Component {
  ...
  getVersionItemMenu = list => {
    const {
      ...
      is_show,
      selectedKeys
    } = this.props //is_show = 是否显示'上传新版本'
    const { defaultSelectedKeys = [] } = this.state
    return (
      <div
        style={{ position: 'relative' }}
        id="versionPanePosition"
        className={indexStyles.versionPanePosition}
      >
        <Menu
          key={selectedKeys}
          ...
          selectable={true}
          style={{ width: 400, maxHeight: '314px' }}
        >
          <div
            key="versionTitle"
            style={{
          ...
          >
            <div
              className={indexStyles.title_wrapper}
              style={{
                display: 'flex',
                alignItems: 'center',
            ...
            >
              ...
              <span
                ...
                className={indexStyles.version_close}
              >
                x
              </span>
            </div>
          ...
          >
            {list.map((value, key) => {
              ...
              return (
                <Menu.Item
                  style={{ height: 'auto' }}
                  className={indexStyles.version_menuItem}
                  key={file_id}
                >
                  {is_edit_version_description && is_edit ? (
                    <div style={{ marginBottom: '5px' }}>
                      <Input.TextArea
                        ...
                        autosize={true}
                        onChange={e => {
                          this.handleChgEditVal(list, e)
                        }}
                        onBlur={() => {
                          this.handleFileDecription(list, file_id)
                        }}
                        onClick={e => {
                          this.handleStopPro(e)
                        }}
                        onKeyDown={e => {
                          this.handleKeyDown(list, file_id, e)
                        }}
                        maxLength={50}
                        value={editValue}
                      />
                    ...
                    <div
                      style={{ position: 'relative' }}
                      className={`${
                        indexStyles.versionItemMenu
                      } ${filePreviewCurrentFileId == file_id &&
                        indexStyles.current_version_color}`}
                    >
                      <div
                        className={`${globalStyles.authTheme} ${indexStyles.circle_icon} ${indexStyles.hover_color}`}
                      >
                        {filePreviewCurrentFileId == file_id ? (
                      ...
                      {remarks && remarks != '' ? (
                        <div style={{ lineHeight: '30px' }}>
                          <span
                            ...
                            className={`${indexStyles.creator} ${indexStyles.hover_color}`}
                          >
                            {remarks}&nbsp;&nbsp;&nbsp;&nbsp;
                          </span>
                          {filePreviewCurrentFileId == file_id && (
                            <span className={`${indexStyles.status}`}>
                              主版本
                            </span>
                          )}
                        </div>
                      ...
                      <span
                        className={`${indexStyles.file_size} ${indexStyles.initalShow}`}
                      >
                        {file_size}
                      </span>
                      <div
                        className={`${indexStyles.file_size} ${indexStyles.initalHide} ${globalStyles.authTheme} ${indexStyles.operate}`}
                      >
                        {is_show === true ? (
                          <Dropdown
                            getPopupContainer={() =>
                              document.getElementById('versionPanePosition')
                            }
                            overlay={this.ellipsisVersionItemMenu({
                              list,
                              file_id,
                              file_name
                            })}
          ...
          >
            <Upload
              className={indexStyles.upload_file}
              {...uploadProps}
            ...
            >
              {is_show === true ? (
                <Button
                  type="primary"
                  style={{ color: '#fff', textAlign: 'center', width: 368 }}
                >
                  <Icon
                    type="upload"
                    theme="outlined"
  ...
  ellipsisVersionItemMenu = ({ list, file_id, file_name }) => {
    return (
      <Menu
        getPopupContainer={() => document.getElementById('versionPanePosition')}
        onClick={e => {
          this.handleVersionChg({ list, file_id, file_name }, e)
        }}
      >
      ...
      <div style={{ position: 'relative' }}>
        <Dropdown
          ...
          onVisibleChange={visible => {
            this.handleVisibleChg(visible)
          }}
          overlay={this.getVersionItemMenu(new_filePreviewCurrentVersionList)}
          trigger={['click']}
        >
          <Button
            className={indexStyles.version}
            style={{
              height: 24,
              marginLeft: 14,
              display: 'flex',
