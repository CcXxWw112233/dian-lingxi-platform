...
class ExcuteTodo {
    constructor(options) {
        const { code, message, id, board_id, undo_id, group_view_type, dispatch, parent_card_id, selected_card_visible, card_detail_id, operate_in_card_detail_panel } = options
        this.id = id //操作对象的id
    ...
    // 拖拽后弹出提示窗
    createNotify = () => {
        const { code, message, id, board_id, undo_id, parent_card_id, selected_card_visible, card_detail_id, dispatch, operate_in_card_detail_panel } = this
...
export class EnequeueNotifyTodos {
    constructor(options) {
        const { code, data, message, id, board_id, group_view_type, dispatch, card_detail_id, selected_card_visible, operate_in_card_detail_panel } = options
        // this.data = data
        // this.code = code
        // this.message = message
    ...
    flush = () => {
        const { group_view_type, dispatch, id, board_id, selected_card_visible, card_detail_id, operate_in_card_detail_panel } = this
        let item
        let excute
        while (item = this.todoQueue.shift()) { //队列遍历
            const { code, message, undo_id, parent_card_id } = item
            excute = new ExcuteTodo({ code, message, undo_id, id, board_id, group_view_type, dispatch, parent_card_id, selected_card_visible, card_detail_id, operate_in_card_detail_panel }) //执行当前一条的弹窗
...
export function rebackCreateNotify({
    ...
    if (code == '0') {
        ...
        if (!this.notify) {
            this.notify = new EnequeueNotifyTodos({ id, board_id, group_view_type, dispatch, card_detail_id, selected_card_visible, operate_in_card_detail_panel })
        }
