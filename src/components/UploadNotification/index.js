...
export default class UploadNotification extends Component {
    ...
    return (
      ...
      <div className={styles.file_out}>
        {uploading_file_list.map(value => {
          ...
          return (
            <div
              className={`${styles.upload_file_item} ${status == 'error' &&
                styles.error}`}
            >
              ...
              <div className={styles.upload_file_item_right}>
                <div
                  className={`${styles.upload_file_item_name} ${globalStyles.global_ellipsis}`}
                >
                  {name}
                </div>
  ...
  renderUploadState = () => {
    ...
    const is_has_uploading =
      uploading_file_list.length &&
      uploading_file_list.findIndex(item => item.status == 'uploading') != -1
    const icon_loading = (
      <i className={globalStyles.authTheme} style={{ color: '#52C41A' }}>
        &#xe7fa;
      </i>
    )
    const icon_upload = (
      <i className={globalStyles.authTheme} style={{ color: '#1890FF' }}>
        &#xe77d;
      </i>
    )
    let message = ''
  ...
  render() {
    ...
    const { minify } = this.state
    return show_upload_notification ? (
      <div
        className={`${globalStyles.global_card} ${styles.notice_out} ${
          minify ? styles.notice_out_mini : styles.notice_out_normal
        }`}
      >
        {minify ? (
          <>
          ...
          <>
            <div className={styles.top}>
              ...
              <div className={styles.message}>
                {' '}
                {this.renderUploadState().message}
              </div>
              <div
                className={`${globalStyles.authTheme} ${styles.close}`}
                onClick={this.setMinify}
            ...
            <div
              className={`${styles.picture_list} ${globalStyles.global_vertical_scrollbar}`}
              ref={this.scroll_ref}
            >
