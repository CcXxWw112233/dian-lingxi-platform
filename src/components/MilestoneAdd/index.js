...
export default class MilestoneAdd extends React.Component {
	...
	renderChildMenu = (currMenu = []) => {
		...
		return currMenu.map((value, key) => {
			...
			return (
				!!(milestone_chird && milestone_chird.length) ?
					<Menu.SubMenu title={
						<div onClick={(e) => {timeName ? '' : this.handleSubMenuClick(e,value)}} className={indexStyles.menuItemDiv} style={{margin: '0px -4px'}}>
					...
					:
					<Menu.Item onClick={({domEvent}) => domEvent && domEvent.stopPropagation() } className={timeName ? `${indexStyles.menuItem} ${indexStyles.disabled}` : `${indexStyles.menuItem}`}
