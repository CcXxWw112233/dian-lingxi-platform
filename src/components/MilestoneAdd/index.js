...
export default class MilestoneAdd extends React.Component {
  ...
  handleSubMenuClick = (e, item) => {
    ...
    if (selectedValue) {
      if (selectedValue == key) {
        this.setSelectKey(obj, 'remove')
      } else {
        this.setSelectKey(obj, 'update')
      }
    } else {
      this.setSelectKey(obj, 'add')
    }
  }
  ...
  setSelectKey(e, type) {
    ...
    this.props.onChangeMilestone &&
      this.props.onChangeMilestone({ key, type, info })
  }
  ...
  submitCreatMilestone = data => {
    ...
    if (
      (dataInfo.due_time && compareTwoTimestamp(dataInfo.due_time, due_time)) ||
      (dataInfo.start_time &&
        compareTwoTimestamp(dataInfo.start_time, due_time))
    ) {
    ...
    createMilestone(params).then(res => {
      if (isApiResponseOk(res)) {
        this.props.getMilestone &&
          this.props.getMilestone(params.board_id, {
            callBackFun: this.getMilestoneListCallbackFun,
            param: res.data
          })
  ...
  getMilestoneListCallbackFun = (milestoneList, param) => {
    ...
    this.props.onChangeMilestone &&
      this.props.onChangeMilestone({ key, type: actionType, info })
  }
  ...
  getSortLilestoneList = (milestoneList, dataInfo) => {
    ...
    let selectableArray = milestoneList.filter(item => {
      return (
        ...
        compareTwoTimestamp(item.deadline, dataInfo.start_time)
      )
    })
  ...
  //递归 渲染子菜单
  renderChildMenu = (currMenu = []) => {
    const { selectedValue, dataInfo = {} } = this.props
    return currMenu.map((value, key) => {
      const { id, name, milestone_chird = [], deadline } = value
      const timeName =
        !compareTwoTimestamp(deadline, dataInfo.due_time) ||
        !compareTwoTimestamp(deadline, dataInfo.start_time)
      return !!(milestone_chird && milestone_chird.length) ? (
        <Menu.SubMenu
          title={
            <div
              onClick={e => {
                timeName ? '' : this.handleSubMenuClick(e, value)
              }}
              className={indexStyles.menuItemDiv}
            ...
            >
              ...
              <div>
                <div
                  style={{
                    display: selectedValue == id ? 'inline-block' : 'none'
                  }}
                >
                  <Icon type="check" />
                </div>
                {timeName && (
                  <Tooltip title="当前任务的开始或截止时间无法超出里程碑截止时间">
                    <div className={indexStyles.menuItemTip}>
          ...
          className={
            timeName
              ? `${indexStyles.menuItem} ${indexStyles.disabled}`
              : `${indexStyles.menuItem}`
          }
              ...
              >
                ...
                >
                  <div
                    ...
                    onClick={() => this.setAddLCBModalVisibile(true)}
                  >
                ...
                {this.renderChildMenu(sortLilestoneList)}
              </Menu>
            </div>
          }
        >
