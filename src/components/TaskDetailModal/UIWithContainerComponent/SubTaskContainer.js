...
import { compareTwoTimestamp, timeToTimestamp, timestampToTime } from '@/utils/util'
...
const SubTaskLogic = {
  ...
  chirldrenTaskChargeChange: function (dataInfo) {
    let sub_executors = []
    const { projectDetailInfoData: { data = [] }, drawContent = {}, dispatch } = this.props
...
const SubTaskItemLogic = {
  ...
  chirldrenTaskChargeChange: function (dataInfo) {
    // let sub_executors = []
    const { projectDetailInfoData: { data = [] }, drawContent = {}, dispatch, childTaskItemValue } = this.props
  ...
  itemOneClick: function () {
    const { childTaskItemValue, dispatch, drawContent: { board_id } } = this.props
  ...
  setchildTaskNameBlur: function () {
    const { dispatch, childTaskItemValue = {}, drawContent: { board_id } } = this.props
  ...
  setChildTaskIndrawContent: function ({ name, value, operate_properties_code }, card_id, rely_card_datas, res) {
    ...
    if ((name && value) || (name && value == null)) {
      ...
      const { group_view_type, drawContent: { board_id } } = this.props
  ...
  startDatePickerChange: function (timeString) {
    ...
    const { milestone_data = {}, card_id: parent_card_id, board_id } = drawContent
    const { data = [] } = drawContent['properties'] && drawContent['properties'].filter(item => item.code == 'MILESTONE').length && drawContent['properties'].filter(item => item.code == 'MILESTONE')[0]
    const { card_id } = childTaskItemValue
    const nowTime = timeToTimestamp(new Date())
    const start_timeStamp = timeToTimestamp(timeString)
  ...
  handleDelStartTime: function (e) {
    e && e.stopPropagation()
    const { dispatch, childTaskItemValue, drawContent: { card_id: parent_card_id, board_id } } = this.props
...
export default class SubTaskContainer extends Component {
  render() {
    const { children, SubTaskUIComponent, handleTaskDetailChange, handleChildTaskChange, whetherUpdateParentTaskTime, updateRelyOnRationList, boardFolderTreeData, handleRelyUploading, updatePrivateVariablesWithOpenFile } = this.props
    return (
      <>
        {
          SubTaskUIComponent ?
