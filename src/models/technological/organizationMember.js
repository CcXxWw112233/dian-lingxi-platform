  ...
  effects: {
    *noLookPermissionsHandle({ payload }, { select, call, put }) {
      const openNotification = () => {
        notification.open({
          message: '提示',
          description: '您所在当前组织没有查看组织团队职员权限。',
          onClick: () => {
      ...
      if (isApiResponseOk(res)) {
        yield put({
          ...
          payload: {
            calback: function() {
              message.success(
                `已将${currentNounPlanFilterName(MEMBERS)}加入组织`,
                MESSAGE_DURATION_TIME
              )
            }
          }
      ...
      if (isApiResponseOk(res)) {
        yield put({
          ...
          payload: {
            calback: function() {
              message.success(
                `已将${currentNounPlanFilterName(MEMBERS)}移除`,
                MESSAGE_DURATION_TIME
              )
            }
          }
      ...
      if (isApiResponseOk(res)) {
        yield put({
          ...
          payload: {
            calback: function() {
              message.success(
                `已将${currentNounPlanFilterName(MEMBERS)}移出该分组`,
                MESSAGE_DURATION_TIME
              )
            }
          }
      ...
      if (isApiResponseOk(res)) {
        yield put({
          ...
          payload: {
            calback: function() {
              message.success(
                `设置${currentNounPlanFilterName(MEMBERS)}分组成功`,
                MESSAGE_DURATION_TIME
              )
            }
          }
        ...
        message.warn(
          `设置${currentNounPlanFilterName(MEMBERS)}分组失败`,
          MESSAGE_DURATION_TIME
        )
      }
      return res
      ...
      if (isApiResponseOk(res)) {
        yield put({
          ...
          payload: {
            calback: function() {
              message.success(
                `邀请${currentNounPlanFilterName(MEMBERS)}加入分组成功`,
                MESSAGE_DURATION_TIME
              )
            }
          }
        ...
        yield put({
          ...
          payload: {
            calback: function() {
              message.success(
                status === '0'
                  ? `已拒绝${currentNounPlanFilterName(MEMBERS)}加入`
                  : '已通过审批',
                MESSAGE_DURATION_TIME
              )
            }
      ...
      if (isApiResponseOk(res)) {
        yield put({
          ...
          payload: {
            calback: function() {
              message.success(
                `已停用该${currentNounPlanFilterName(MEMBERS)}`,
                MESSAGE_DURATION_TIME
              )
            }
          }
