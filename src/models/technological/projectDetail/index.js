  ...
  effects: {
    ...
    *historyListenSet({ payload }, { select, call, put }) {
      ...
      const appsSelectKeyIsAreadyClickArray =
        ...
        []
      if (!appsSelectKeyIsAreadyClickArray.length) {
        //如果还没有点过app,即从其他页面进来
        yield put({
          //查询项目角色列表
    ...
    *initProjectDetail({ payload }, { select, call, put }) {
      ...
      if (isApiResponseOk(result)) {
        ...
        if (!appsSelectKey) {
          ...
          yield put(
            routerRedux.replace(
              `/technological/projectDetail?board_id=${board_id}&appsSelectKey=${result.data.app_data[0].key}`
            )
          )
        } else {
          if (entry == 'editProjectApp') {
            //编辑app
            yield put({
              type: 'handleEditApp',
              payload: {
    ...
    // 获取项目动态列表
    *getProjectDynamicsList({ payload }, { select, call, put }) {
      // 获取项目动态列表
    ...
    *getProjectRoles({ payload }, { select, call, put }) {
      const res = yield call(getProjectRoles, {
        ...payload,
        _organization_id:
          payload._organization_id || getGlobalData('aboutBoardOrganizationId')
      })
      if (isApiResponseOk(res)) {
        yield put({
          type: 'updateDatas',
