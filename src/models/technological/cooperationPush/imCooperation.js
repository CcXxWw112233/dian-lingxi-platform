...
export default {
  ...
  effects: {
    *getImUnReadAllMessages({ payload }, { select, call, put }) {
      //获取和设置全部未读消息
      const { messages = [] } = payload
      console.log('ssss_初始化首次传递', messages)
      const wil_handle_types = yield select(
        getModelSelectState('imCooperation', 'wil_handle_types')
      )
    ...
    *imUnReadMessageItemClear({ payload }, { call, put, select }) {
      //未读消息清除,场景（当用户点开某一条具有红点的消息后，会清除该条消息）
      const { relaDataId } = payload
      let im_all_latest_unread_messages = yield select(
        getModelSelectState('imCooperation', 'im_all_latest_unread_messages')
      )
      im_all_latest_unread_messages = im_all_latest_unread_messages.filter(
        item => !!item
      )
      const idServer = (
        im_all_latest_unread_messages.find(
    ...
    *listenImLatestAreadyReadMessages({ payload }, { select, call, put }) {
      //在触发的已读推送,已读后更新未读列表
      const { messages = [] } = payload
      ...
      const arr = im_all_latest_unread_messages.filter(item => {
        if (messages.findIndex(item2 => item2 == item.idServer) != -1) {
          //传递进来的已读列表不包含该条未读消息
          return item
        }
      })
      const reads = arr.map(item => ({
    ...
    *listenImUnReadLatestMessage({ payload }, { select, call, put }) {
      //获取最新的一条未读消息推送
    ...
    *imMessageToRead({ payload }, { call, put }) {
      //im的某一条消息设置已读
    ...
    // 处理甘特图
    *handelGanttCardCreate({ payload }, { call, put }) {
      //im的某一条消息设置已读
      const { message_item = {} } = payload
