...
export default {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen(location => {
        ...
        //头部table key
        if (
          location.pathname.indexOf('technological/simplemode/workbench') !== -1
        ) {
        }
      })
    }
  ...
  effects: {
    ...
    *getFileList({ payload }, { call, put }) {
      ...
      const res = yield call(getBoardFileList, {
        folder_id,
        board_id: board_id
      })
      //console.log("res", res);
      ...
      if (isApiResponseOk(res)) {
        ...
        yield put({
          ...
          payload: {
            breadcrumbList: [
              {
                file_name: res.data.folder_name,
                file_id: res.data.folder_id,
                type: '1'
              }
            ],
